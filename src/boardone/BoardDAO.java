...
public class BoardDAO {
	...
	public void insertArticle(BoardVO article) {
		...
		ResultSet rs = null;
		// 답변글일 경우를 대비해서 미리 num, ref, step, depth를 복사 = 답변글 1번 작업
		int num = article.getNum();
		int ref = article.getRef();
		int step = article.getDepth();
		int depth = article.getDepth();
		int number = 0; // 새글일 경우 사용할 임시변수
		String sql = "";
		...
		try {
			...
			// 답변 글일 경우
			if(num != 0) {
				sql = "update board set step=step+1 where ref= ? and step > ?"; // 답변글 2번 작업
