...
public class StudentDAO {
	...
	public String getPass(String id) {
		...
		try {
			conn = getConnection(); // db랑 connection 연결
			pstmt = conn.prepareStatement(strQuery); // sql 실행 할라고 
			pstmt.setString(1, id); // ? 넣어주는거
			rs = pstmt.executeQuery(); // query문 실행하고 받아옴
	...
	public int deleteMember(String id, String pass) {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String dbPass = getPass(id); // 데이터베이스에 실제 저장된 패스워드
		int result = -1; // 결과치
